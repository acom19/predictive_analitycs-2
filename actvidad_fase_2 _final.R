...
datos_limpios$Country <- fct_lump(datos_limpios$Country, 
                                    n = 1) # Se establece la cantidad de categorías adicional a la de referencia
...
sapply(df_km_std, function(x) if(is.numeric(x)) sd(x, na.rm = TRUE) else NA)


summary(df_km_std)

# --------------------------------------------------------------------------------
# ******Efectuar prueba PCA para el modelo kmeans
# Ejecutar PCA
pca_result <- prcomp(df_km_std, center = TRUE)

# Resumen de los resultados de PCA
summary(pca_result)
#summary(pca_result2)

# Cargas (contribuciones de cada variable a cada PC)
#pca_result$rotation

# Mostrar las cargas de las variables en los primeros 4 componentes que son los relevantes
pca_result$rotation[, 1:4]  # todas las filas y las cariables 1 a 4

# eliminar la variable country.other
df_km_std <- df_km_std %>% 
...
wcss <- vector()
for(i in 1:10){
  wcss[i] <- sum(kmeans(df_km_std, i)$withinss)
}
...
# preparar datos para la matriz de Spearman
datos_spearman <- df_km_std %>%
  ungroup() %>% # desagrupar el dataset datoscliente
  select(-cluster) # eliminar estas variables para dejar solo numéricas
